...
// tag::spotify-repeat-off-json-docs[]
constexpr char kDesiredRepeatState[] = "off";

void sendSpotifyRepeatOffJson(WebSocketsClient& socket) {
  String body;
...
void handleSpotifyRepeatOffJsonResponse(uint8_t* payload, size_t length) {
...
// end::spotify-repeat-off-json-docs[]

void websocketEvent(WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
    case WStype_CONNECTED:
      Serial.println("WS_CONNECTED");
      sendSpotifyRepeatOffJson(webSocket);
      break;
    case WStype_TEXT:
      handleSpotifyRepeatOffJsonResponse(payload, length);
      break;
    default:
      break;
  }
