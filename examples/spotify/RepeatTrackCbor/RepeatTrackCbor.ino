...
// tag::spotify-repeat-track-cbor-docs[]
constexpr char kDesiredRepeatState[] = "track";

void sendSpotifyRepeatTrackCbor(WebSocketsClient& socket) {
  uint8_t encoded[96];
...
void handleSpotifyRepeatTrackCborResponse(uint8_t* payload, size_t length) {
...
// end::spotify-repeat-track-cbor-docs[]

void websocketEvent(WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
    case WStype_CONNECTED:
      Serial.println("WS_CONNECTED");
      sendSpotifyRepeatTrackCbor(webSocket);
      break;
    case WStype_BIN:
      handleSpotifyRepeatTrackCborResponse(payload, length);
      break;
    default:
      break;
  }
